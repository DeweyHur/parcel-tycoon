...
(function (root) {
  ...
  const SCENARIOS = {
    ...
    peak:     { name: '성수기', icon: '🎄', months: 2, desc: '입고 ×1.6, 폭주 턴 3회. 보상 +10, 가격 ×1.3, 시작 호출 +2', win: '2개월 생존 + 34개 처리', mods: { arrivalsMult: 1.6, burstTurns: 3, rewardAll: 10, itemPriceMult: 1.3, startCallsDelta: 2, winDelivered: 34 }, unlock: 'busy_month', recommend: '도심 퀵 · 강철 창고' },
    heatwave: { name: '폭염', icon: '🌡️', months: 3, desc: '신선 비율 +15%p, 상온 부패 3배, 폭염 경보 턴(월 2회)', win: '3개월 생존 + 폐기 3개 이하', mods: { typeShift: { fresh: 15, normal: -15 }, freshSizes: [2, 4, 7], warmMult: 3, heatAlerts: 2, facilityPriceMult: { cold1: 0.7, cold2: 0.7 }, marketWeight: { cold: 1.5 }, winMaxDiscard: 3 }, unlock: 'fresh20', recommend: '프레시 로지스틱스' },
    strike:   { name: '파업', icon: '✊', months: 3, desc: '매월 운송 업체 1종 호출 불가 (월초 공지). 긴급 특송 상시 등장', win: '3개월 생존', mods: { strike: true, guaranteeCarrier: 'urgent' }, unlock: 'four_carriers', recommend: '스타트업' },
    port:     { name: '항만 계약', icon: '🚢', months: 4, desc: '국제 20%·대형 12%, 초대형 6%. 국제·대형 보상 +20, 일반 -5', win: '4개월 생존', mods: { typeOverride: { normal: 40, fresh: 15, fragile: 13, intl: 20, large: 12 }, xlWeight: 6, rewardDelta: { intl: 20, large: 20, normal: -5 }, xlDelta: 1, guaranteeCarriers: ['intl', 'large'] }, unlock: 'intl15', recommend: '글로벌 · 강철 창고' },
    cashcrunch:{ name: '자금난', icon: '💸', months: 3, desc: '시작 자금 -50%, 운영비 200, 새로고침 불가. 수익 +10%, 신뢰도 ×2', win: '3개월 생존 + 자금 500 이상', mods: { cashMult: 0.5, opCostFixed: 200, noRefresh: true, revenueMult: 1.1, trustXpMult: 2, winCash: 500 }, unlock: 'rich_clear', recommend: '짠돌이 · 국영 우편' },
  ...
  const ACHIEVEMENTS = {
    ...
    three_companies:{ name: '런 5회', desc: '서로 다른 회사 3개로 런 클리어', kind: 'meta', rewardType: 'company', reward: 'startup', check: (s, p) => Object.keys(p.clearsByCompany).length >= 3 },
    long_service: { name: '장기 근속', desc: '반기 결산 클리어', kind: 'end', needWin: true, rewardType: 'company', reward: 'postal', check: (s, p, r) => r.scenario === 'half' },
    // 시나리오·슬롯 해금
    first_clear:  { name: '첫 클리어', desc: '아무 시나리오나 클리어', kind: 'end', needWin: true, rewardType: 'multi', reward: ['scenario:half', 'slot:2'], check: () => true },
    busy_month:   { name: '바쁜 달', desc: '한 달에 15개 이상 처리', kind: 'run', rewardType: 'scenario', reward: 'peak', check: s => s.maxMonthDelivered >= 15 },
    fresh20:      { name: '신선 스무 개', desc: '신선식품 누적 20개 처리', kind: 'cum', rewardType: 'scenario', reward: 'heatwave', check: (s, p) => p.deliveredByType.fresh >= 20 },
    ...
    intl15:       { name: '국제 열다섯', desc: '국제운송 누적 15개 처리', kind: 'cum', rewardType: 'scenario', reward: 'port', check: (s, p) => p.deliveredByType.intl >= 15 },
    ...
    three_unlocked:{ name: '세 회사', desc: '회사 3개 해금', kind: 'meta', rewardType: 'scenario', reward: 'daily', check: (s, p, r, prof) => prof.unlocked.companies.length >= 3 },
    veteran:      { name: '베테랑', desc: '회사 5개 해금', kind: 'meta', rewardType: 'slot', reward: 3, check: (s, p, r, prof) => prof.unlocked.companies.length >= 5 },
    // 퍽 해금
    ...
    waiter:       { name: '기다리는 자', desc: '대기 누적 40회', kind: 'cum', rewardType: 'perk', reward: 'foresight', check: (s, p) => p.waits >= 40 },
    ...
    // 기록용
    perfect_month:{ name: '완벽한 달', desc: '페널티 0으로 한 달 마감', kind: 'run', rewardType: 'none', check: s => s.perfectMonths >= 1 },
