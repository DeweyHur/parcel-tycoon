...
(function () {
  ...
  function showMarket() {
    ...
    const render = () => {
      ...
      const fcRows = game.customerForecast().map(f => { const cu = M.CUSTOMERS[f.id]; return `<span class="fc">${cu.icon} ${esc(cu.name)} <b>${f.min}~${f.max}</b>${f.special > 0 && f.types.length ? ` <small>${f.types.map(t => `<i style="display:inline-block;width:7px;height:7px;background:${D.PARCEL_TYPES[t].css}"></i>${D.PARCEL_TYPES[t].short}`).join(' ')} ${Math.round(f.special * 100)}%</small>` : ''}</span>`; }).join('');
