...
(function (root) {
  ...
  ui: {
    ...
    'mk.forecastSum': "{m}개월차 예상 물량 {min}~{max}개",
    'mk.forecast': "{m}개월차 예상 물량(대략)",
    'log.monthStart': '── {y}년 {cal}월 · {m}개월차 시작 ──',
    ...
    'log.storagePerTurn': '이틀에 {perTurn}c 후불',
    ...
    'how.2.d': '트럭 값은 <b>부를 때마다</b> 나갑니다. 두 개만 실으면 개당 16c, 꽉 채우면 개당 4c. <b>안 부르고 기다리는 것이 정답일 때가 많습니다.</b>',
    'how.3.t': '1년을 버틴다',
    'how.3.d': '3월에 인수해서 내년 2월까지. 돈이 마르거나 창고가 포화 상태가 되면 끝입니다.',
    ...
    'hud.monthEnd': '월말 정산',
    'hud.weekend': '주말',
    'fmt.date': '{cal}월 {a}~{b}일',
    'fmt.dayRange': '{a}~{b}일',
    'log.weekend': '🛌 주말 ({a}~{b}일) — {choice}{extra}',
    'err.noOutdoor': '마당에 나가 있는 게 없습니다',
    'story.weekend.1': '주말이야. 우리 창고는 주말에도 물건이 들어오지만, 트럭은 안 움직여.<br>그러니까 주말엔 부를 수가 없어. 기한도 같이 멈추니까 그건 걱정 말고.',
    'story.weekend.2': '문제는 마당이야. 밖에 둔 건 주말 이틀 내내 거기 있어 — 도둑 한 번 더 맞는다고 보면 돼.<br>금요일에 마당 비우는 게 습관이 돼야 해. 자, 이번 주말은 뭘 할래?',
    'wk.title': '🛌 주말',
    'wk.head': '{cal}월 {a}~{b}일 · 주말',
    'wk.sub': '주말엔 차를 부를 수 없습니다. 기한도 멈춥니다 — 다만 <b>마당에 둔 물건은 도난 판정을 한 번 더</b> 받습니다.',
    ...
    'wk.rest.done': '쉬었다',
    'wk.overtime': '야근',
    'wk.overtime.d': '다음 영업일 직접 배송 +2 · 스트레스 +1',
    'wk.overtime.done': '야근했다',
    'wk.parttime': '주말 알바',
    'wk.parttime.d': '{cost}c · 이번 주말 도난 없음',
    ...
    'story.intro.1': '어서 와. 오늘부터 여기 사장은 자네야.<br>이 회사는 1년 단위로 돌아. {startCal}월에 시작해서 <b>내년 {lastCal}월까지 버티면 자네가 이긴 거야.</b><br>나는 석 달만 옆에서 잔소리하고 빠질게. 듣기 싫으면 저 ×를 눌러.',
    ...
    'story.callReady.1': '이제 차 부를 만해. {ready} 눌러 봐.<br>차 한 대가 {readyCap}칸인데, 게이지가 80% 넘게 차면 손해 안 보는 선이야.',
    'story.callReady.2': '눌러 보면 호출 창이 떠. 거기서 뭘 누를지는 내가 하나씩 짚어 줄게.',
    ...
    'story.callModal.1': '버튼에 <b>실을 것 {openVol}/{openCap}칸 · {openFee}c</b> 이렇게 적혀 있었지?<br>차 한 대가 {openCap}칸인데 지금 이 계약으로 실을 게 {openVol}칸 있다는 뜻이야. 한 대 부를 때마다 {openFee}c 나가고.<br>위 게이지가 그 {openVol}/{openCap}을/를 그림으로 보여주는 거야.',
    'story.callGo.1': '담겼지? 게이지가 80% 넘으면 남는 장사야. 아래 목록을 눌러서 넣고 뺄 수도 있어.',
    'story.callGo.2': '됐으면 호출 버튼. 호출하면 그대로 이틀이 지나가.',
    ...
    'story.usage76.2': '위에 \'입고 예정\' 보이지? 앞으로 나흘에 뭐가 얼마나 오는지 미리 보여.<br>그걸 보고 기다릴지 부를지 정해.',
    ...
    'story.market1.2': '카드마다 차 용량·대당 배차비·배차 대수가 적혀 있어. 배차는 쓰면 줄고 월초에 안 채워져 — 여기서 \'가득 충전\'만 돼. 위에 \'다음 달 예상\' 줄 보이지? 어떤 택배가 몇 개쯤 올지야.<br>그걸 보고 부족한 차를 채워.',
    'story.market1.3': '프리미엄 등급은 비싸. 돈 남을 때 얘기고, 지금은 표준으로 충분해.<br>4월엔 고객이 특수 물건을 슬슬 맡기기 시작하니까 냉장 구역은 비워 둬.',
    ...
    'story.loss.2': '보험이 있으면 일부는 돌려받아. 없으면 마켓에서 보험사를 골라 둬.<br>스트레스도 올랐을 거야 — 게이지가 끝까지 차면 끝이니까.',
    ...
    'story.win.1': '이 동네 택배는 1년 단위로 돌아. {startCal}월에 시작했으니까 내년 {lastCal}월까지.',
    'story.win.2': '{lastCal}월 물량까지 넘기면 그때부터 자네가 진짜 사장이야. 점수니 뭐니는 그다음 얘기고.',
    'story.win.3': '그 전에 빚이 한도를 넘거나 스트레스가 끝까지 차면… 뭐, 다시 하면 되지.',
    ...
    'story.farewell.1': '석 달 됐네. 이제 나 없어도 돌아가.',
    'story.farewell.2': '달력은 사무실 벽에 붙여 뒀어. 9월 추석, 11월 행사, 12월 연말 — 이 셋만 조심해.<br>위에 달 숫자를 누르면 언제든 볼 수 있어.',
    'story.farewell.3': '급하면 문자할게. 커피는 자네 몫이야.',
    'story.startTitle': '인수인계',
    'story.startBody': '동네 택배를 물려받아 한 해를 돕니다. 전임 창고장 <b>박 반장</b>이 첫 석 달 동안 옆에서 규칙을 하나씩 알려 줍니다.<br>강제는 아니에요 — 대화는 언제든 건너뛸 수 있고, 메뉴에서 끌 수 있습니다.',
    'story.diffAsk': '난이도',
    'story.start': '시작',
    ...
    'cal.kr.2.label': '설',
    'cal.kr.2.note': '설 — 물량 +30%, 선물세트(신선·농산·파손) ↑, 3~10일 폭주, 13~16일 연휴',
    ...
    'cal.kr.5.note': '가정의 달 — 물량 +5%, 선물(파손주의) ↑, 13~20일 선물 주간(⚠ 보상 +10)',
    'cal.kr.5.sms': '',
    ...
    'cal.kr.11.note': '김장철·쇼핑 행사 — 물량 +30%, 농산물 ↑, 19~28일 행사 주간(일반 폭주, 대량 배차비 −10%)',
    ...
    'offer.perTurn': '이틀에 {perTurn}c 후불 ({total}c)',
    ...
    'demo.gateBody': '데모는 인수인계 {n}개월까지 플레이할 수 있습니다. 본편에서는 이어지는 달의 성수기·명절 폭주와 물가 상승까지 한 해를 끝까지 운영합니다.',
    ...
    'demo.resultBody': '{n}개월을 버텼습니다. 본편은 같은 창고에서 한 해를 끝까지 갑니다.',
  ...
  data:   {
    ...
    "CARRIERS": {
      ...
      "sea0": {
        ...
        "short": "파도",
        "vehicle": "컨테이너",
        "desc": "통관 컨테이너, 크기 2 이상, 파손 안전. 보상은 나흘 뒤 입금 · 표준 계약"
      },
      "sea1": {
        "name": "대양 컨테이너",
        "short": "대양",
        "vehicle": "컨테이너",
        "desc": "통관 컨테이너, 크기 2 이상, 파손 안전. 보상은 나흘 뒤 입금 · 프리미엄 — 배차 +1·용량 +1·배차비 −10% · 입금 이틀 지연"
      }
    },
    "FAMILIES": {
      "bulk": { "name": "대량" },
  ...
  meta:   {
    ...
    "WEATHER": {
      ...
      "snow": {
        "name": "폭설",
        "desc": "야외 ❄ 부패 없음·❆ 이틀 유예, 도난 절반, ❄ 기한 정지, ❄ 처리 +10"
      },
      "storm": {
        "name": "태풍",
    ...
    "DIFFICULTIES": {
      "rookie": {
        "name": "수습",
        "desc": "입고 ×0.85, 운영비 -30, 배차비 ×0.8, 가격 ×0.9, 도난·파손·배상 ×0.5, 예정 6일, 스트레스 한계 24. 점수 ×0.7. 해금 도전과제 인정 안 됨"
      },
      "normal": {
        "name": "정규",
      ...
      "veteran": {
        "name": "베테랑",
        "desc": "입고 ×1.15, 운영비 +60, 배차비 ×1.3, 도난·파손 ×1.3, 배상 ×1.5, 속성 2개 택배 +3%p, 스트레스 한계 16. 점수 ×1.4"
      }
    },
    "COMPANIES": {
      "local": {
      ...
      "fresh": {
        "name": "프레시 로지스틱스",
        "tag": "신선식품 특화",
        "passive": "콜드체인: 신선식품 기한 +이틀. 냉장 물류 호출 시 신뢰도 +1 추가",
      ...
      "glass": {
        "name": "유리방 물류",
        "tag": "파손주의 · 정밀",
        "passive": "완충 포장: 파손주의 처리 기한 +나흘, 보상 +15",
      ...
      "postal": {
        "name": "국영 우편",
        "tag": "안정형 · 장기전",
        "passive": "공공 서비스: 운영비 230·배차비 70c 고정. 직접 배송 +1개. 스트레스 10 이상이면 월초 -2",
    ...
    "PERKS": {
      ...
      "freezer": {
        "name": "냉동고",
        "desc": "냉장 구역의 신선식품은 첫 나흘 동안 부패 정지"
      },
      "yard": {
        "name": "야적장",
    ...
    "SCENARIOS": {
      ...
      "peak": {
        "name": "성수기",
        "desc": "달력의 11~12월 컷(4개월차 물량으로 시작). 쇼핑 행사·연말 물량 위에 폭주 3회(기한 −나흘), 반송 유예 나흘. 보상 +10, 가격 ×1.3, 시작 배차 +2",
      ...
      "blackfriday": {
        "name": "블랙 프라이데이",
        "desc": "11월 한 달 지옥. 쇼핑 행사 위에 입고 ×1.4, 폭주 3회, 가격 ×1.4, 반송 유예 8일. 보상 +15, 시작 배차 +3",
        "win": "11월 생존 + 30개 처리",
        "recommend": "도심 퀵 · 짠돌이"
      },
      "audit": {
        "name": "감사",
        "desc": "모든 기한 -이틀, 기한 초과 보상 -50%, 월초 스트레스 +1",
        "win": "12개월 생존 + 기한 초과 처리 12개 이하",
        "recommend": "국영 우편 · 유리방"
      },
      "moving": {
        "name": "이사철",
        "desc": "이사센터 포함, 보관 제안 열흘마다 보장, 보관료 ×1.5",
