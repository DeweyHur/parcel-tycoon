...
(function (root) {
  ...
  ui: {
    ...
    'media.flyer': "Local flyers",
    'media.sns': "Social ads",
    'media.radio': "Local radio",
    'media.bus': "Bus & billboards",
    'media.upName': "{name} upgrade Lv.{lv}",
    ...
    'growth.automation': "Automation",
    'growth.branding': "Brand",
    'growth.coldchain': "Cold chain",
    'growth.desc.fleet': "+1 call on every contract",
    'growth.desc.warehouse': "+{n} warehouse cells",
    'growth.desc.automation': "Fees -4% · labor -8%",
    'growth.desc.branding': "Cargo value +3% · premium +4%",
    'growth.desc.coldchain': "Cold +2 · frozen +1",
    'growth.head': "Growth",
    'camp.half': 'overlap {n}',
    ...
    'hv.day': 'A day',
    'hv.call': 'Calling a carrier',
    'hv.kind': 'Parcel types',
    'hv.early': 'Deadlines · trust · reputation',
    'hv.wh': 'Warehouse',
    'hv.wx': 'Weather',
    'hv.camp': 'Campaigns',
    'hv.pay': 'Settlement · market',
    'hv.run': 'Runs and the calendar',
    'hv.flow.in': 'arrivals',
    'hv.flow.call': 'call',
    'hv.flow.wait': 'no call',
    'hv.flow.tick': 'deadline −1',
    'hv.day.week': 'Open Mon–Sat, closed Sunday',
    'hv.day.cycle': 'Settle every half-month → market',
    'hv.call.tap': 'Tap a contract card — the most urgent parcels load <b>until the truck is full</b>',
    'hv.call.box': 'Tap parcels to add or remove',
    'hv.call.btn': 'The big button becomes <b>Call</b> · tap the card again to cancel',
    'hv.call.half': 'The fee is per truck — fill it <b>past {pct}%</b> to profit',
    'hv.call.two': 'Overflow adds a truck by itself — up to <b>{n}</b> per call',
    'hv.call.pips': 'Blue bars = dispatches left — they run down, and a renewal in the market refills them',
    'hv.kind.size': 'Size',
    'hv.kind.normal': 'Any truck',
    'hv.kind.fresh': 'Outside cold storage: gone next day',
    'hv.kind.frozen': 'Outside the freezer: gone at once',
    'hv.kind.fragile': 'Breaks 25% on trucks that are not break-safe',
    'hv.kind.produce': 'Heat wave: deadline −2',
    'hv.kind.intl': '2 days in customs after arrival',
    'hv.kind.large': 'Only trucks that fit the size',
    'hv.early.fast': 'Right away',
    'hv.early.ok': 'Comfortably',
    'hv.early.due': 'On the deadline',
    'hv.early.late': 'Late',
    'hv.trust.lv': 'Carrier trust — average parcel score per truck (−2~+2) · Lv {a} / {b} / {c}xp',
    'hv.rep.rule': 'Reputation — same sum ÷{div} (±{max}) · clean settlement +{clean} · discards/theft −',
    'hv.due.late': 'Past the deadline: reward −{pct}% · {n} more days and it is returned',
    'hv.wh.main': 'Main',
    'hv.wh.cold': 'Cold',
    ...
    'hv.pay.op': 'Operating cost {n}c/month + contract and facility upkeep',
    'hv.pay.note': 'Notes turn into cash when they mature',
    'hv.pay.loan': 'Short of cash? A short-term loan ({pct}% interest, up to {limit}c)',
    'hv.pay.market': 'Market after settlement — contracts, upgrades, facilities',
    'hv.run.q': 'Spring',
    'hv.run.q2': 'Summer · Autumn · Winter',
    'hv.run.half': 'Half year',
    'hv.run.year': 'Full year',
    'hv.run.holiday': '5 days before Seollal/Chuseok: arrivals ×1.6; no calls on the holidays',
    'camp.runs': "Runs",
    ...
    'camp.desc': "Pull in parcels when the warehouse is empty. A campaign pass is used up once run — buy more in the settlement market.",
    'camp.btn': "Run a campaign",
    'camp.effect': "{n} more parcels within {days} working days",
    'camp.used': "No runs left this half-month",
    ...
    'jackpot.sweep': 'CLEAN SWEEP! All {n} shipped',
    'jackpot.rush': '⚡ ALL RUSH OUT SAME DAY!',
    'ps.rushToday': '⚡×{x}',
    ...
    'mk.feeEach': "{n}c per truck",
    'mk.upgrade': "Upgrade",
    'mk.emptySlots': "{n} empty slot(s)",
    'mk.fcNow': "This half",
    'mk.fcNext': "Next half",
    'mk.insChange': "Change",
    ...
    'slot.block.size': 'Vehicle too big (size-4-or-less contracts only)',
    'slot.block.none': 'Not applicable',
    'mk.newContracts': "New contracts",
    'mk.noEmptySlot': "no empty slot — buying replaces a contract",
    'sum.notesPaid': "Notes matured",
    'sum.notesCount': "{n} notes",
    'sum.notesLeft': "Notes outstanding",
    ...
    'log.paid': "📄 {name} note matured +{amount}c ({count} parcels)",
    ...
    'log.callDelay': ' (📄 {delay:half-month note|1-month note})',
    ...
    'market.dealRenew': '{name} renewal',
    'mk.sign': 'Sign',
    'mk.signed': 'Signed',
    'deal.cycles': '{n} cycles',
    'deal.cells': '~{n} slots/cycle ±{sp}%',
    'deal.rel': 'relation Lv{n}',
    'deal.fan': '{name} partner — more volume, better rate',
    'deal.fanList': 'Partner insurers: {list}',
    'deal.line': '📄 {left}/{cycles} cycles · this cycle {got}/{target} · on time {pct}% · ⚡{rush} · incidents {claims}',
    'deal.none': 'No contract — offers come in the settlement market',
    'deal.head': 'Business contracts',
    'deal.anonHead': 'Walk-in customers — base volume that grows with reputation',
    'deal.rules': '📄 Businesses offer contracts at settlement · 🛡 insurance required (partner insurer = better terms) · graded S/A/B/C at expiry → relation & reputation · 🎁 bonus for finishing without incidents',
    'deal.result': '{icon} {name} contract ended — grade {grade} ({score}){bonus}{rel}',
    'deal.resultBonus': ' · 🎁 +{n}c',
    'deal.resultRel': ' · relation Lv{n}',
    'err.deal.insurance': '🛡 You need insurance to sign',
    'err.deal.slots': 'Up to {n} business contracts at once',
    'err.deal.dup': 'Already under contract',
    'log.dealSign': 'Business contract: {name}, {cycles} cycles · ~{cells} slots per cycle',
    'log.dealDone': '{name} contract ended — grade {grade}{bonus}',
    'log.dealBonus': ' · no-incident bonus +{n}c',
    'why.repDeal': '{name} contract grade {grade}',
    'deal.relLbl': 'Relation',
    'sum.secDeal': 'Contracts ended',
    'hv.deal': 'Customers · business contracts',
    'hv.deal.anon': 'Walk-in volume is the base — it grows with reputation',
    'hv.deal.offer': 'Businesses send <b>offers</b> in the settlement market: expected volume (±{sp}%) for {a}–{b} cycles. Signing is free',
    'hv.deal.ins': '<b>Insurance required</b>. Holding their partner insurer raises volume and rate',
    ...
    'repi.up': '▲ 🚚 80%+ full · 🤝 customer trust · ✅ clean settlement',
    'repi.down': '▼ ↩ returns · 🕵 theft · 💥 breakage · 🥀 spoilage · 📦 overflow · ⏳ late',
    'repi.nextHead': 'At {name}',
    'repi.nextScale': 'Arrivals +{arr}% · running costs +{op}%',
    'repi.nextCust': 'New customers: {list}',
    'repi.nextGoods': 'New goods: {list}',
    'repi.zero': 'At zero nobody sends you anything — the run ends.',
    'cust.upHint': 'more trust → more parcels & pay',
    ...
    'why.repEarly': 'shipped ahead of deadline',
    'why.repLate': 'shipped late',
    ...
    'rep.greet.fragile': "{center}, Ahn speaking. {vehicle}, {cap} slots, {fee}c a truck, {trucks} trucks. ⚠ never breaks in our vans — we'll take standard and produce too, but ⚠ is where you make money.",
    'rep.greet.fragile.1': "{center}, Ahn here! Still a {cap}-slot van, but plenty of trucks — {trucks}, {fee}c each. Save up your ⚠ and send it all at once — I bring my own bubble wrap.",
    ...
    'log.refill': 'Renewed: {name} {n} trucks (-{price}c){wasted}',
    'log.refillWasted': ' · {n} leftover trucks wasted',
    'mk.refillDesc': 'Renew to full (same price however many remain).',
    ...
    'cd.refillLine': 'Renew {price}c',
    'cd.calls': 'Calls',
    'toast.refill': 'Renewed: {name} {n} trucks',
    ...
    'story.usage.1': 'The warehouse sign reads <b>Warehouse 12/24</b> — cells used. Half full is comfortable, past three quarters you start paying attention, full means boxes pile up outside.<br>Outside is… a story for later.',
    ...
    'story.callModal.3': 'Tapping a card loads the most urgent first, but <b>only as far as a full truck</b>. Tap parcels to add or remove them.<br>Calling a half-empty second truck just burns another dispatch fee. If you still have to send it, pick it by hand.',
    ...
    'coach.wait': 'Dispatching is done today. {vol} cells now, a {name} truck holds {cap} — <b>skip calling and receive the next arrivals</b>.',
    ...
    'story.callGo.2': 'Ready? Hit the <b>Call</b> button below. Calling ends the day. Tap the card again to send the truck away and just rest.',
    ...
    'outro.1': 'Spring is over. Same spot as three months ago, same shutter.',
    'outro.2': "— “Just keep it rolling like always.” That's what Mr. Han said, handing me the key.",
    'outro.3': '“{name}”. From today, the sign has my name on it.',
    'outro.4': 'Park will turn up with coffee again tomorrow morning.',
    'outro.5': 'And the trucks will come tomorrow, too.',
    'outro.end': '— Handover complete —',
    'intro.skip': 'Skip ▸',
    ...
    'story.l1call.1': 'Now it is worth calling — <b>{readyVol}/{readyCap}</b> worth already.<br>Tap the <b>Hangil</b> card below; that brings the truck in.',
    'story.l1callGo.1': "Yeo, from Hangil Logistics. One call runs <b>{nowFee}c</b>.<br>The painted cells are what we will take away.",
    'story.l1callGo.2': "The urgent ones are <b>already loaded</b>, as many as fit. Tap boxes to add or remove.<br>Call now: <b>{nowRev}c</b> in, {nowFee}c fee — <b>{nowNet}c left</b>. Hit <b>Call</b> below. Tap the card again to send the truck away instead.",
    ...
    'story.l2callsOut.2': "Just this once I talked Yeo into <b>topping it up</b>.<br>It does not refill by itself. From now on you <b>buy it at the market</b> — I will show you after the settlement.",
    ...
    'story.l3invest.1': 'The depot sat empty two days running. Trucks and hands idle — nothing costs more.',
    'story.l3invest.2': 'That is when you run a <b>campaign</b>. Flyers, ads — you pull the work in. Tap it.',
    'story.l3campGo.1': "I kept one flyer for you — that's a campaign pass. Use it and parcels pour in within days. Open it.",
    'story.l3campDone.1': 'Look at tomorrow and the day after — more, right? A pass is gone once used. Buy another in the settlement market.',
    'story.l3campDone.2': 'But careful — if cargo you pulled in with ads gets returned, word spreads twice as bad. <b>Only pull in what you can handle</b>, and only when the depot is about to go empty.',
    ...
    'story.l2pack.1': 'And this. <b>Gentle Parcel</b> — a small van contract.<br>Next half brings more volume. Hangil alone will run out of calls.',
    ...
    'story.l3intro.3': 'Remember <b>Gentle Parcel</b> from the last market? ⚠ fragile starts arriving this month.<br>The van takes ⚠ and plain alike, but at 4 cells a plain-only load barely breaks even.',
    'story.l3fragile.1': 'See the ⚠? That one breaks. Hangil will carry it, but with no padding it <b>may break</b>.',
    ...
    'story.l3trust.3': 'It climbs the <b>earlier</b> you ship before the deadline — same-day is best — and drops when you ship late. Each tier adds a perk.',
    ...
    'story.l4cold.2': 'See the <b>chilled</b> sign inside with its count? You don\u2019t want that full.',
    ...
    'story.l6intro.2open': 'Use the breath to get ready. February brings the New Year rush, and that\u2019s the last wall.<br>Clear it and you\u2019re done \u2014 balance settled and all.',
    ...
    'story.l6frozen.2': 'See the frozen sign? Frozen has fewer cells than chilled. Send those first.',
    ...
    'letter.6.good': 'You saw it through.<br><br>I trusted you from the start, and you did it splendidly.<br>Now make that depot grow.',
    'letter.6.bad': 'A few were dropped, but you saw it through.<br><br>I trusted you from the start, and you got there.<br>Now make that depot grow.',
    ...
    'ct.seller': 'Seller (Party A)',
    'ct.sellerName': 'Mr. Han',
    'ct.buyer': 'Buyer (Party B)',
    'ct.item': 'Property',
    'ct.itemName': "Depot No.1, Sinheung-dong 17-3, with yard",
    'ct.body': "Art. 1 (Price) The purchase price of the property is {price}c.<br>Art. 2 (Deposit) Party B pays Party A a deposit of {down}c on the day of signing.<br>Art. 3 (Balance) The balance of {rest}c is paid in instalments, one every half-month, within two months of signing. No interest.",
    ...
    'inst.cleared': 'Paid in full. Grateful to Mr. Han for the chance.',
    'inst.btn': 'Send it',
    'inst.btnFinal': 'Take the deed',
    'ct.titleFinal': 'B I L L   O F   S A L E',
    'ct.bodyFinal': 'Art. 1 (Payment in full) Party A has received the full purchase price of {price}c from Party B.<br>Art. 2 (Transfer) Party A transfers all ownership of the property to Party B.',
    ...
    'call.xpShort': 'this call <b>+{xp}xp</b>',
    'call.trustHold': 'hold for the trust tiers',
    ...
    'call.payLater': "📄 {n:half-month note|1-month note}",
    'call.bonus': 'Bonus',
    'call.selected': "Selected <b>{n}</b>/{cap}",
    'call.pickNone': 'Tap parcels to load',
    ...
    'wm.vans': 'Vehicles',
    'wm.noVans': 'No vehicles — ⚠ fragile may break',
    'wm.blocked': '{n} not eligible (see parcel details)',
    'wm.title': 'End Day',
    'wm.selfWait': '🚐 Self-deliver {n} (-{cost}c)',
    ...
    'sum.instalment': 'Instalment (Mr. Han)',
    'sum.instalmentNote': 'at chapter end · leaves {left}c',
    'sum.opCostDetail': 'rent {rent} · contracts {contracts} · facilities {facilities} · labor {labor} ({arrivals} arrivals)',
    'hud.feesDue': "fees −{n}",
    'hud.cashLbl': "Balance",
    'hud.cashNow': "cash {n}",
    'hud.pending': "notes due +{n}",
    'hud.stock': 'stock +{n}',
    'hud.opCostDue': "op cost & premium −{n}",
    'hud.instalment': "instalment −{n}",
    ...
    'feat.surge': 'Month {m} peak',
    'feat.low': 'Month {m} lull',
    'feat.orders': 'orders ×{x}',
    'feat.ordersDays': 'orders ×{x} · {n} days',
    'feat.xDays': '×{x} · {n} days',
    'feat.off': '{n}-day holiday, no trucks',
    'feat.rush': '{h} rush',
    'feat.h.seol': 'Seollal',
    'feat.h.chuseok': 'Chuseok',
    'feat.rain': 'Monsoon',
    'feat.rainFx': 'rain ×{x}',
    'feat.heat': 'Heatwave alert',
    'feat.heatFx': 'heat ×{x}',
    'feat.snow': 'Blizzards',
    'feat.snowFx': 'snow ×{x}',
    'feat.moving': 'Moving season',
    'feat.movingFx': 'storage requests ×{x}',
    'feat.sale': 'Shopping festival',
    'feat.gift': 'Gift week',
    'feat.giftFx': '⚠ reward +{v}',
    'title.rank': 'Leaderboard',
    'rank.title': 'Leaderboard',
    'rank.loading': 'Loading…',
    'rank.fail': 'Could not load the leaderboard',
    'rank.empty': 'No scores yet',
    'rank.total': '{n} players',
    'rank.res': '{board}: #{r} of {n}',
    'rank.newBest': 'New best',
  ...
  data:   {
    ...
    "ENHANCEMENTS": {
      ...
      "seal": {
        "name": "Trust Seal",
        "desc": "Trust xp +2 on the chosen contract"
      },
      "record": {
        "name": "Long-term Record",
        "desc": "Trust xp +4 on the chosen contract"
      },
      "optFragile": {
  ...
  meta:   {
    ...
    "PERKS": {
      ...
      "shelves": {
        "name": "Extra Shelves",
        "desc": "Start with a shelf rack installed (+8 cells, size 2 or less)",
        "short": "Start with a shelf rack"
      },
      "freezer": {
        "name": "Freezer",
