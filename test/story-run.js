...
(async () => {
  ...
  for (let i = 0; i < 8; i++) { const g = await page.evaluate(() => !!PT.game); if (g) break; const b = await page.$('.foot .btn.primary'); if (!b) break; await b.click(); await page.waitForTimeout(400); }
  ...
  while (guard++ < 400) {
    ...
    if (st.phase === 'weekend') {
      await page.screenshot({ path: `shots/r-weekend-m${st.m}-t${st.t}.png` });
      await readBeat();
      await page.click('.wkopts .wkc:not([disabled])', { force: true }); await page.waitForTimeout(600); await readBeat(); continue;
    }
    if (st.phase === 'summary' || st.phase === 'market') {
      if (st.phase === 'market') await page.screenshot({ path: `shots/r-market-m${st.m}.png` });
      await page.click('.foot .btn.primary', { force: true }); await page.waitForTimeout(500); continue;
    }
    ...
    if (act >= 0) {
      ...
      for (const b of await page.$$('.modal .body .btn')) { if (/급한|Auto/.test(await b.textContent())) { await b.click({ force: true }); await page.waitForTimeout(100); break; } }
      const go = await page.$('.foot .btn.primary:not([disabled])');
      if (go) { await go.click({ force: true }); await page.waitForTimeout(900); await readBeat(); continue; }
      await page.click('.foot .btn', { force: true }); await page.waitForTimeout(100);
    }
    await page.click('#wait-btn', { force: true }); await page.waitForTimeout(200);
    const w = await page.$('.foot .btn.primary'); if (w) { await w.click({ force: true }); await page.waitForTimeout(450); }
    await readBeat();
  }
