...
t('시작 상태', () => { const g = NG(1); assert.equal(g.cash, 600); assert.equal(g.contracts.length, 4); assert.equal(g.phase, 'play'); assert.equal(g.month, 1); });
...
t('대기는 호출 횟수를 차감하지 않음', () => { const g = NG(3); const calls = g.contracts.map(c => c.calls); g.wait(); assert.deepEqual(g.contracts.map(c => c.calls), calls); assert.equal(g.turn, 2); });
...
t('처리량 초과 선택 거부', () => { const g = NG(5); for (let i = 0; i < 8 && g.phase === 'play'; i++) g.wait(); const idx = g.contracts.findIndex(c => c.carrier === 'target'); const c = g.contracts[idx], cap = g.callCapacity(c), elig = g.eligibleParcels(c); if (elig.length > cap) { const r = g.callCarrier(idx, elig.slice(0, cap + 1).map(p => p.id)); assert.ok(!r.ok); } });
t('스킵 보너스: 대기 후 처리량 +1 (동네 단골 +1 포함)', () => { const g = NG(7); const c = g.contracts[3]; assert.equal(g.callCapacity(c), 5); g.wait(); assert.equal(g.callCapacity(c), 6); const h = new Game({ seed: 7, company: 'postal', perks: ['skip'] }); const b = h.contracts[1]; assert.equal(h.callCapacity(b), 4); h.wait(); assert.equal(h.callCapacity(b), 5); });
...
t('회사별 시작 상태', () => {
  ...
  const q = new Game({ seed: 2, company: 'quick' }); assert.equal(q.contracts[0].maxCalls, 8);
