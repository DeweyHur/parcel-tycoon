...
증빙으로 보관할 것은 `audio-src/*.wav` 원본이며, **git 에 함께 커밋한다**(6곡 약 59MB — GitHub 의
파일당 100MB 한도에 한참 못 미치고, 레포 전체도 100MB 아래다). 자격증명이 살아 있는 사본이
히스토리에 남는 편이 백업 하나보다 낫다. 다만 원본을 교체할 때마다 같은 용량이 히스토리에 영구히
더해지므로, 재다운로드는 필요할 때만 한다.
...
- [x] `suno-terms-2026-09-13.pdf` / `.txt` — 다운로드 시점의 약관 전문(2026-09-13T01:28:37Z 캡처).
      현행 약관의 발효일(2026-09-03)이 다운로드일(09-13)보다 앞서므로 이 판본이 적용 버전이다.
      브라우저에서 뽑은 렌더 텍스트이며, 페이지에서 계산한 SHA-256
      `4a433b7b40426cc2df26e74440da545ccb9ea7677cdecf8f998771de1eb15b27` 과 전송 후 재검증해 일치
- [x] `audio-src/*.wav` — 자격증명이 살아 있는 원본. git 에 커밋됨
- [x] 이 파일과 `docs/MUSIC_MANIFEST.md` (git 에 커밋돼 있음)

인보이스·영수증 PDF에는 집 주소와 카드 끝 4자리가 들어 있어 git 에 올리지 않는다
(`docs/licenses/.gitignore`). 약관 캡처는 개인정보가 없지만 같은 폴더 규칙을 따라 로컬에만 둔다.
